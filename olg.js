...
client.on("message", async message => {
  ...
  if(command === "help") {
    const embedMessage = new Discord.MessageEmbed()
      ...
      .addFields(
        { name: prefix+'help', value: 'Show all commands for the bot, you can say "commands" and it will post voice commands.'},
        { name: prefix+'ping', value: 'Pings the bot and send the response time.' },
        { name: prefix+'join', value: 'Joins the current voicechannel of the person who sent the command.' },
        { name: prefix+'play', value: 'Plays the song you write after the commad.' },
  ...
  if(command === "add") {
    ...
    for (const connection of client.voice.connections.values()) {
      if (connection.speaking) {
        ...
        let embed = setEmbed(searchResults.items[0].title, searchResults.items[0].bestThumbnail.url, message.author.username, searchResults.items[0].url);
        await message.channel.send("**" + searchResults.items[0].title + "**" + " added to the queue!", {
          embed: embed
        });
      } else {
        getYtID(args.join(" "))
                        ...
                        if (keywords.includes(firstWord)) {
                            if (firstWord == "play" || firstWord == "lay") {
                                ...
                                if (connection.speaking && queue.length) {
                                  ...
                                  client.channels.cache.get(botChannel).send("Queue is empty!");
                                }
                            } else if (firstWord == "stop") {
                              if (connection.speaking) {
                                connection.destroy();
                              }
                            } else if (firstWord == "add" || firstWord == "queue"|| firstWord == "q") {
                              if (connection.speaking) {
                                queue.push(noKeyWord.join(' '))
                                const searchResults = await ytsr(noKeyWord.join(' '));
                                let embed = setEmbed(searchResults.items[0].title, searchResults.items[0].bestThumbnail.url, message.author.username, searchResults.items[0].url);
                                client.channels.cache.get(botChannel).send("**" + searchResults.items[0].title + "**" + " added to the queue!", {
                                  embed: embed
                                });
                              } else {
                                getYtID(noKeyWord.join(' '))
                              ...
                              .addFields(
                                { name: 'commands', value: 'Show all commands for the bot, you can say "help" and it will post voice commands.'},
                                { name: 'play', value: 'Plays the song you say after the commad.' },
                                { name: 'add/queue', value: 'Add a song to the queue.' },
                              ...
                              client.channels.cache.get(botChannel).send(embedMessage);
                            }
                        } else {
                            // exec('rm *.pcm');
                            console.log("Inget");
...
async function getYtID(q) {
  ...
  const embedMessage = new Discord.MessageEmbed()
	.setColor('#0099ff')
	.addFields(
		{ name: 'Now playing:', value: '['+ searchResults.items[0].title +']('+ searchResults.items[0].url +')' }, 
	)
	.setThumbnail(searchResults.items[0].bestThumbnail.url)
  

  client.channels.cache.get(botChannel).send("**" + searchResults.items[0].title + "**", {
    embed: embedMessage
  });
  status = true
  ...
  const dispatcher = connection.play(await ytdld(searchResults.items[0].url, {
...
function setEmbed(title, thumbnail="", user, url) {
   const embedMessage = new Discord.MessageEmbed()
	.setColor('#0099ff')
	.addFields(
		{ name: 'Added to queue:', value: '['+ title +']('+ url +')' }, 
	)
